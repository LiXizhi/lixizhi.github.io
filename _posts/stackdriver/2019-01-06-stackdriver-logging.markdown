...
필자는 로깅 에이전트를 On-Prem 환경에서 실행해보았는데, 정상적으로 동작하지 않았다. 로깅 에이전트는 GCP에서만 동작하는 것으로 판단하고 여기서는 `SpringBoot 2.x`를 이용해서 Application을 작성하고, `로깅 라이브러리`로 로그를 전송하는 방법을 소개하겠다.
<br><br>
